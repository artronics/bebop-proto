...
resource "aws_cloudwatch_event_rule" "ecs_event_stream" {
  name        = "${local.name_prefix}-ecs-event-stream"
  description = "Passes ecs event logs to lambda that writes them to cw logs"

  event_pattern = <<PATTERN
  {
    "detail": {
      "clusterArn": ["${aws_ecs_cluster.project_cluster.arn}"]
    }
  }
PATTERN
}
...
resource "aws_lambda_permission" "ecs_event_stream" {
  statement_id  = "${local.name_prefix}_AllowExecutionFromCloudWatch"
...
resource "aws_lambda_function" "ecs_event_stream" {
  function_name    = "${local.name_prefix}-ecs-event-stream"
  ...
  "widgets": [
    {
      ...
      "height": 18,
      "properties": {
        "query": "SOURCE '/aws/lambda/${local.name_prefix}-ecs-event-stream' | fields @timestamp as time, detail.desiredStatus as desired, detail.lastStatus as latest, detail.stoppedReason as reason, detail.containers.0.reason as container_reason, detail.taskDefinitionArn as task_definition\n| filter @type != \"START\" and @type != \"END\" and @type != \"REPORT\"\n| sort detail.updatedAt desc, detail.version desc\n| limit 100",
        "region": "us-east-1",
        "title": "ECS Event Log"
      }
    }
